...
**Feature Branch**: `001-neon-auth-migration`  
**Created**: 2025-11-23  
**Status**: Draft  
**Input**: User description: "migrate from betterauth to neon auth and use expo auth tools for mobile auth to neon auth. Also, we are using expo launch instead of codemagic so no more betterauth and no more codemagic."
...
### User Story 3 - Cutover and decommission legacy auth (Priority: P3)

There are no current users on the legacy system. The team fully removes legacy authentication components and associated CI/CD tooling, verifies clean builds and deployments, and confirms that the new sign-in flow works in all environments.

**Why this priority**: Ensures a clean cutover without residual complexity, reducing maintenance and risk.

**Independent Test**: Remove legacy auth and CI/CD configuration, run a full build and deployment using the current pipeline, and verify new-user sign-in works in dev, staging, and production.

**Acceptance Scenarios**:

1. Given legacy authentication code and configuration exist, When they are removed and the project is rebuilt, Then builds and deployments succeed without reference to legacy components.
2. Given the new authentication system is active, When a new user signs in on each environment, Then sign-in succeeds and protected routes are accessible, with no legacy dependencies involved.
...
- FR-007: The system MUST remove dependencies on the legacy authentication provider and any legacy CI/CD tooling references associated with it.
...
- FR-013: There are no current legacy users; the system MUST remove legacy authentication components and related CI/CD configuration entirely without requiring any account linking.
...
- Assumes mobile delivery uses the current mobile distribution pipeline and does not depend on the legacy CI/CD tool.
...
- SC-006: No dependencies on the legacy authentication provider or its associated CI/CD tooling remain in any environment.
